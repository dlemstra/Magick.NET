...
namespace ImageMagick
{
  internal static partial class MagickExceptionHelper
  {
    ...
    {
      ExceptionSeverity severity = (ExceptionSeverity)NativeMethods.Severity(exception);
      string message = NativeMethods.Message(exception);
      string description = NativeMethods.Description(exception);
    ...
    private static List<MagickException> CreateRelatedExceptions(IntPtr exception)
    {
      ...
      int nestedCount = NativeMethods.RelatedCount(exception);
      if (nestedCount == 0)
        return result;
      ...
      {
        IntPtr nested = NativeMethods.Related(exception, i);
    ...
    public static MagickException Create(IntPtr exception)
    {
      ...
      NativeMethods.Dispose(exception);
    ...
    public static bool IsError(MagickException exception)
    {
      ...
      return type.IsSubclassOf(typeof(MagickErrorException)) || type == typeof(MagickErrorException);
    }
  }
}
