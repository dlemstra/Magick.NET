...
namespace ImageMagick
{
	...
	internal sealed class ExifWriter
	{
		...
		private int WriteValue(ExifDataType dataType, object value, Byte[] destination, int offset)
		{
			switch (dataType)
			{
				...
				case ExifDataType.SingleFloat:
					return Write(BitConverter.GetBytes((Single)value), destination, offset);
				default:
		...
		public Byte[] GetData()
		{
			...
			if (exifLength > 0)
				_Values[exifIndex].Value = (uint)(ifdOffset + ifdLength);

			if (gpsLength > 0)
				_Values[gpsIndex].Value = (uint)(ifdOffset + ifdLength + exifLength);
