    ...
    private sealed class NativeMagickImageCollection : NativeHelper
    {
      ...
      public void Map(MagickImage image, QuantizeSettings settings, MagickImage remapImage)
      {
        using (INativeInstance settingsDynamic = QuantizeSettings.CreateInstance(settings))
        {
          IntPtr exception = IntPtr.Zero;
          if (NativeLibrary.Is64Bit)
            NativeMethods.X64.MagickImageCollection_Map(MagickImage.GetInstance(image), settingsDynamic.Instance, MagickImage.GetInstance(remapImage), out exception);
          else
            NativeMethods.X86.MagickImageCollection_Map(MagickImage.GetInstance(image), settingsDynamic.Instance, MagickImage.GetInstance(remapImage), out exception);
      ...
      {
        using (INativeInstance settingsDynamic = MontageSettings.CreateInstance(settings))
        {
          IntPtr exception = IntPtr.Zero;
          IntPtr result;
          if (NativeLibrary.Is64Bit)
            result = NativeMethods.X64.MagickImageCollection_Montage(MagickImage.GetInstance(image), settingsDynamic.Instance, out exception);
          else
            result = NativeMethods.X86.MagickImageCollection_Montage(MagickImage.GetInstance(image), settingsDynamic.Instance, out exception);
          MagickException magickException = MagickExceptionHelper.Create(exception);
          if (MagickExceptionHelper.IsError(magickException))
          {
            if (result != IntPtr.Zero)
      ...
      public void Quantize(MagickImage image, QuantizeSettings settings)
      {
        using (INativeInstance settingsDynamic = QuantizeSettings.CreateInstance(settings))
        {
          IntPtr exception = IntPtr.Zero;
          if (NativeLibrary.Is64Bit)
            NativeMethods.X64.MagickImageCollection_Quantize(MagickImage.GetInstance(image), settingsDynamic.Instance, out exception);
          else
            NativeMethods.X86.MagickImageCollection_Quantize(MagickImage.GetInstance(image), settingsDynamic.Instance, out exception);
          CheckException(exception);
        }
      }
