  ...
  public partial class MagickColor
  {
    private static class NativeMethods
    {
      public static class X64
      {
        ...
        [DllImport(NativeLibrary.X64Name, CallingConvention = CallingConvention.Cdecl)]
        [return: MarshalAs(UnmanagedType.Bool)]
        public static extern bool MagickColor_Initialize(IntPtr Instance, [MarshalAs(UnmanagedType.CustomMarshaler, MarshalTypeRef = typeof(UTF8Marshaler))] string value);
      }
    ...
    private sealed class NativeMagickColor : NativeInstance
    {
      ...
      {
        using (INativeInstance otherDynamic = MagickColor.CreateInstance(other))
        {
          if (NativeLibrary.Is64Bit)
            return NativeMethods.X64.MagickColor_FuzzyEquals(Instance, otherDynamic.Instance, fuzz);
          else
            return NativeMethods.X86.MagickColor_FuzzyEquals(Instance, otherDynamic.Instance, fuzz);
        }
      }
      public bool Initialize(string value)
      {
        if (NativeLibrary.Is64Bit)
          return NativeMethods.X64.MagickColor_Initialize(Instance, value);
        else
          return NativeMethods.X86.MagickColor_Initialize(Instance, value);
      }
    }
