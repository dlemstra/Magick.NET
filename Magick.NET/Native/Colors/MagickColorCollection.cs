    ...
    private static class NativeMethods
    {
      ...
      public static void DisposeList(IntPtr list)
      {
        if (NativeLibrary.Is64Bit)
          X64.MagickColorCollection_DisposeList(list);
        else
          X86.MagickColorCollection_DisposeList(list);
      }
      public static IntPtr GetInstance(IntPtr list, int index)
      {
        if (NativeLibrary.Is64Bit)
          return X64.MagickColorCollection_GetInstance(list, (UIntPtr)index);
        else
          return X86.MagickColorCollection_GetInstance(list, (UIntPtr)index);
      }
    }
  }
}
