...
function copyLibraries($source, $target) {
    ...
    Copy-Item "$source\**\content\win\**\**\*.dll" -Force "$target\win"
    ...
    Copy-Item "$source\**\content\osx\**\**\*.dylib" -Force "$target\osx"
}
...
function createTrademarkAttribute($source, $target) {
    ...
    $imageMagickVersion = (Get-Item "$source\**\content\\win\ReleaseQ8\x64\*.dll").VersionInfo.FileVersion
