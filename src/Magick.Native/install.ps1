...
function installPackage($version, $target) {
    ...
    $url = "https://dl.dropboxusercontent.com/s/6gqi06zrj6q479b/Magick.Native.$version.nupkg"
...
function copyLibraries($source, $target) {
    ...
    [void](New-Item -ItemType directory -Path "$target\windows")
    Copy-Item "$source\**\content\windows\**\**\*.dll" -Force "$target\windows"
    ...
    [void](New-Item -ItemType directory -Path "$target\macos")
    Copy-Item "$source\**\content\macos\**\**\*.dylib" -Force "$target\macos"
}
...
function createTrademarkAttribute($source, $target) {
    ...
    $imageMagickVersion = (Get-Item "$source\**\content\\windows\ReleaseQ8\x64\*.dll").VersionInfo.FileVersion
...
$windowsLibraries = "$libraries\windows"
