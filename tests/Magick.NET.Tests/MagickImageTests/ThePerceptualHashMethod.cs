...
public partial class MagickImageTests
{
    public class ThePerceptualHashMethod
    {
        ...
        public void ShouldReturnThePerceptualHash()
        {
            ...
            AssertChannel(channel, 3, 3.6612, 2.3355);
            AssertChannel(channel, 4, 7.3922, 5.2277);
            AssertChannel(channel, 5, 6.1112, 2.8360);
            AssertChannel(channel, 6, 7.4883, 3.9636);
#elif Q16
            AssertChannel(channel, 0, 0.2599, 0.2570);
            AssertChannel(channel, 1, 3.2672, 1.0553);
            AssertChannel(channel, 2, 3.5845, 1.3801);
            AssertChannel(channel, 3, 3.6532, 2.8361);
            AssertChannel(channel, 4, 7.3708, 5.1087);
            AssertChannel(channel, 5, 6.0809, 3.4734);
            AssertChannel(channel, 6, 7.4908, 5.0815);
#else
            AssertChannel(channel, 0, 0.2599, 0.2946);
            AssertChannel(channel, 1, 3.2672, 1.1834);
            AssertChannel(channel, 2, 3.5845, 1.5010);
            AssertChannel(channel, 3, 3.6532, 3.0420);
            AssertChannel(channel, 4, 7.3707, 5.3652);
            AssertChannel(channel, 5, 6.0808, 3.6675);
            AssertChannel(channel, 6, 7.4909, 5.6501);
#endif
            ...
            AssertChannel(channel, 4, 7.3855, 2.5020);
            AssertChannel(channel, 5, 5.6999, 1.4574);
            AssertChannel(channel, 6, 7.0588, 2.2044);
#elif Q16
            ...
            AssertChannel(channel, 4, 7.3768, 2.5015);
            AssertChannel(channel, 5, 5.6949, 1.4575);
            ...
            AssertChannel(channel, 6, 8.7421, 10.7048);
#elif Q16
