...
public partial class MagickImageTests
{
    public class TheContrastStretchMethod
    {
        ...
        public void ShouldThrowExceptionWhenPercentageIsNull()
        {
            ...
            {
                Assert.Throws<ArgumentException>("blackPoint", () =>
                {
                    image.ContrastStretch(new Percentage(-1));
                });
            }
        }
        ...
        public void ShouldThrowExceptionWhenBlackPointIsNull()
        {
            using (var image = new MagickImage())
            {
                Assert.Throws<ArgumentException>("blackPoint", () =>
                {
                    image.ContrastStretch(new Percentage(-1), new Percentage(50));
                });
            }
        }
        ...
        public void ShouldThrowExceptionWhenWhitePointIsNull()
        {
            using (var image = new MagickImage())
            {
                Assert.Throws<ArgumentException>("whitePoint", () =>
                {
                    image.ContrastStretch(new Percentage(50), new Percentage(-1));
                });
            }
        }
